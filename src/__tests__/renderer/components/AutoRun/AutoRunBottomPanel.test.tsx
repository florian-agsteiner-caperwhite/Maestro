...
describe('AutoRunBottomPanel', () => {
	...
	describe('goal-driven mode', () => {
		const goal = { progress: 45, iteration: 3, rationale: 'Wired the parser into the engine' };

		it('renders "Goal: N%", iteration, and rationale when goal is set', () => {
			renderPanel({ goal });
			expect(screen.getByText('Goal: 45%')).toBeInTheDocument();
			expect(screen.getByText('iteration 3')).toBeInTheDocument();
			expect(screen.getByText('Wired the parser into the engine')).toBeInTheDocument();
		});

		it('hides the task-count readout even when taskCounts are non-zero', () => {
			// Goal mode takes precedence: a stale task count must not leak through.
			renderPanel({ goal, taskCounts: { completed: 2, total: 5 } });
			expect(screen.getByText('Goal: 45%')).toBeInTheDocument();
			expect(screen.queryByText(/of/)).not.toBeInTheDocument();
			expect(screen.queryByText(/tasks/)).not.toBeInTheDocument();
		});

		it('does not render the reset button in goal mode', () => {
			renderPanel({ goal, taskCounts: { completed: 3, total: 5 }, isLocked: false });
			expect(screen.queryByTitle(/Reset.*completed/)).not.toBeInTheDocument();
		});

		it('colors the percent with accent below 100%', () => {
			renderPanel({ goal: { ...goal, progress: 45 } });
			expect(screen.getByText('Goal: 45%')).toHaveStyle({ color: mockTheme.colors.accent });
		});

		it('colors the percent with success at 100%', () => {
			renderPanel({ goal: { ...goal, progress: 100 } });
			expect(screen.getByText('Goal: 100%')).toHaveStyle({ color: mockTheme.colors.success });
		});

		it('omits the iteration label before the first report (iteration 0)', () => {
			renderPanel({ goal: { ...goal, iteration: 0 } });
			expect(screen.queryByText(/iteration/)).not.toBeInTheDocument();
		});

		it('omits the rationale text when none is reported', () => {
			renderPanel({ goal: { progress: 20, iteration: 1, rationale: null } });
			expect(screen.getByText('Goal: 20%')).toBeInTheDocument();
			// Only the percent + iteration render; no empty rationale node.
			expect(screen.getByText('iteration 1')).toBeInTheDocument();
		});

		it('still renders the token count alongside the goal readout', () => {
			renderPanel({ goal, tokenCount: 1500 });
			expect(screen.getByText('Goal: 45%')).toBeInTheDocument();
			expect(screen.getByText('Tokens:')).toBeInTheDocument();
			expect(screen.getByText('1500')).toBeInTheDocument();
		});

		it('truncates the rationale gracefully (truncate class + hover title)', () => {
			const longRationale =
				'A very long rationale that would otherwise overflow the compact bottom panel and break the layout';
			renderPanel({ goal: { ...goal, rationale: longRationale } });
			const rationaleEl = screen.getByText(longRationale);
			// The truncate class + title give graceful overflow handling without wrapping.
			expect(rationaleEl).toHaveClass('truncate');
			expect(rationaleEl).toHaveAttribute('title', longRationale);
		});

		it('uses a tighter rationale max-width in compact mode', async () => {
			// Drive the ResizeObserver with a sub-350px width so the panel flips to
			// compact and the rationale max-width shrinks (graceful, not broken).
			...
			class NarrowResizeObserver {
				...
				observe = (target: Element) => {
					const entry = {
						target,
						contentRect: { width: 300, height: 40 },
			...
			try {
				renderPanel({ goal });
				const rationaleEl = await screen.findByText('Wired the parser into the engine');
				expect(rationaleEl).toHaveStyle({ maxWidth: '120px' });
			} finally {
